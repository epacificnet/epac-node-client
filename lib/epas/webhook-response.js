...
const logger = { debug: require('debug')('epac:epac-node')};
const {validateVerb, specs } = require('@epac/verb-specifications');
...
class WebhookResponse {
   ...
   * the incoming request was signed by tin
   * @param {string} secret - webhook signing secret
   * @param {object} opts - opts
   * @returns function
   */
  static verifyepacSignature(secret) {
    return (req, res, next) => {
      const header = req.get('epac-Signature');
      if (!header) throw new Error(`missing Signature in ${req.url}`);
